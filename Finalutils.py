...
def initializeTrackbars(initialTrackbarVals):
    try:
        cv2.namedWindow("Trackbars")
        cv2.resizeWindow("Trackbars", 360, 240)
        cv2.createTrackbar("Width Top", "Trackbars", initialTrackbarVals[0], 50, nothing)
        cv2.createTrackbar("Height Top", "Trackbars", initialTrackbarVals[1], 100, nothing)
        cv2.createTrackbar("Width Bottom", "Trackbars", initialTrackbarVals[2], 50, nothing)
        cv2.createTrackbar("Height Bottom", "Trackbars", initialTrackbarVals[3], 100, nothing)
...
def sliding_window(img, nwindows=15, margin=50, minpix=1, draw_windows=True):
    ...
    #Initial coeffients for left and right lanes.
    left_fit_ = np.empty(3)
    right_fit_ = np.empty(3)
    #Copy of a image which is filled with white 
    out_img = np.dstack((img, img, img)) * 255

    histogram = get_hist(img)
    # leftx_base and rightx_base represent the initial x-coordinates (base positions) for the left and right lanes. These are calculated by finding the peak pixel count in the left and right halves of the histogram, respectively.
    midpoint = int(histogram.shape[0] / 2)
    leftx_base = np.argmax(histogram[:midpoint])
    rightx_base = np.argmax(histogram[midpoint:]) + midpoint

    # Set height of windows. based on number of windows
    ...
    for window in range(nwindows):
        ...
        if len(good_left_inds) > minpix:
            leftx_current = int(np.mean(nonzerox[good_left_inds]))  # Change np.int to int
        if len(good_right_inds) > minpix:
            rightx_current = int(np.mean(nonzerox[good_right_inds]))  # Change np.int to int
    ...
    if leftx.size and rightx.size:
        ...
        #Average of last 10 polynomial coefficients is calculated and updated to left_fit_ and right_fit_ for smoothing.
...
#This is used to Check for the curve in the Road. 
...
#Used to draw the Lane on the road 
...
#This is used to stack the images into single frame. 
...
#It is Used to draw lines on bottom of the Image.The green line specifies Center.
